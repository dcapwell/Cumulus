 ...
 * while load balancing.  Load balancing is done via {@link com.ekaqu.cumulus.loadbalancer.CollectionLoadBalancer#get()}
 ...
 */
@Beta
public final class CollectionLoadBalancer<E> extends ForwardingCollection<E> implements LoadBalancer<E>, Supplier<E> {

  /**
   * delegated collection to load balance.
   */
  private final Collection<E> items;

  /**
   * Delegated load balancer.
   */
  private final LoadBalancer<E> delegateLoadBalancer;

  /**
   * Filters out data in the {@link #items}.
   */
   ...
   */
  private CollectionLoadBalancer(final Collection<E> items, final LoadBalancer<E> delegateLoadBalancer,
                                 final Predicate<E> filterPredicate) {
   ...
   * @param delegateLoadBalancer used for all load balancer operations
   * @param <E> element type
   * @return new collection load balancer
   */
  public static <E> CollectionLoadBalancer<E> create(final Collection<E> items,
                                                     final LoadBalancer<E> delegateLoadBalancer) {
