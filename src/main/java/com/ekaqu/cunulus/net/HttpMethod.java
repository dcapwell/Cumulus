...
/**
 * Defines the set of Http Methods.  Methods are defined in <a href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9">RFC-2616</a>.
 * Documentation is copied here.
 * <p/>
 * 9 Method Definitions
 * <p/>
 * The set of common methods for HTTP/1.1 is defined below. Although this set can be expanded, additional methods cannot
 * be assumed to share the same semantics for separately extended clients and servers.
 * <p/>
 * The Host request-header field (section <a href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.23">14.23</a>)
 ...
 * taking an action other than retrieval. These methods ought to be considered "safe". This allows user agents to
 * represent other methods, such as POST, PUT and DELETE, in a special way, so that the user is made aware of the fact
 ...
 * GET request; in fact, some dynamic resources consider that a feature. The important distinction here is that the user
 ...
 * of N > 0 identical requests is the same as for a single request. The methods GET, HEAD, PUT and DELETE share this
 * property. Also, the methods OPTIONS and TRACE SHOULD NOT have side effects, and so are inherently idempotent.
 * <p/>
 * However, it is possible that a sequence of several requests is non- idempotent, even if all of the methods executed
 * in that sequence are idempotent. (A sequence is idempotent if a single execution of the entire sequence always yields
 * a result that is not changed by a reexecution of all, or part, of that sequence.) For example, a sequence is
 * non-idempotent if its result depends on a value that is later modified in the same sequence.
 * <p/>
 * A sequence that never has side effects is idempotent, by definition (provided that no concurrent operations are being
...
public enum HttpMethod {
   ...
   * <p/>
   * body is not defined by this specification, but might be defined by future extensions to HTTP. Content negotiation
   * MAY be used to select the appropriate response format. If no response body is included, the response MUST include a
   ...
   * the Request-URI refers to a data-producing process, it is the produced data which shall be returned as the entity
   ...
   * entity be transferred only under the circumstances described by the conditional header field(s). The conditional
   * GET method is intended to reduce unnecessary network usage by allowing cached entities to be refreshed without
   ...
   * <p/>
   * - Annotation of existing resources; - Posting a message to a bulletin board, newsgroup, mailing list, or similar
   * group of articles; - Providing a block of data, such as the result of submitting a form, to a data-handling
   * process; - Extending a database through an append operation. The actual function performed by the POST method is
   * determined by the server and is usually dependent on the Request-URI. The posted entity is subordinate to that URI
   * in the same way that a file is subordinate to a directory containing it, a news article is subordinate to a
   ...
   * which describes the status of the request and refers to the new resource, and a Location header (see section <a
   * href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.30">14.30</a>).
   * <p/>
   ...
   * that URI. If a new resource is created, the origin server MUST inform the user agent via the 201 (Created)
   * response. If an existing resource is modified, either the 200 (OK) or 204 (No Content) response codes SHOULD be
   * sent to indicate successful completion of the request. If the resource could not be created or modified with the
   * Request-URI, an appropriate error response SHOULD be given that reflects the nature of the problem. The recipient
   * of the entity MUST NOT ignore any Content-* (e.g. Content-Range) headers that it does not understand or implement
   ...
   * MAY be overridden by human intervention (or other means) on the origin server. The client cannot be guaranteed that
   * the operation has been carried out, even if the status code returned from the origin server indicates that the
   * action has been completed successfully. However, the server SHOULD NOT indicate success unless, at the time the
   * response is given, it intends to delete the resource or move it to an inaccessible location.
   * <p/>
   ...
   * <p/>
   * origin server or the first proxy or gateway to receive a Max-Forwards value of zero (0) in the request (see section
   * 14.31). A TRACE request MUST NOT include an entity.
   * <p/>
   * TRACE allows the client to see what is being received at the other end of the request chain and use that data for
   * testing or diagnostic information. The value of the Via header field (section <a
   * href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.45">14.45</a>) is of particular interest, since
